import type { CommandDefinition, ApolloClient } from './types.js';
...
export async function executeCommand(
  ...
  if (!cmdDef.fieldMappings) {
    throw new Error(
      `executeCommand requires fieldMappings on '${cmdDef.name}' — use a custom handler or define mappings.`,
    );
  }
